...
<script>
	...
	function checkUsername(){
		...
		// 3.打开连接
		xhr.open("GET","${pageContext.request.contextPath}/user_getUsername.action?time="+new Date().getTime()+"&username="+username,true);
		// 4.发送
		xhr.send(null);
	}
	...
	function change(){
		...
		img1.src="${pageContext.request.contextPath}/checkImage.action?time="+new Date().getTime();
	}
</script>
</head>
<body>
						...
						<table>
							...
							<tr>
								...
								<tr>
									...
									<td>
										<span class="fieldSet">
											<input type="text" id="checkcode" name="checkcode" class="text captcha" maxlength="4" autocomplete="off"><img id="checkImg" class="captchaImage" src="${pageContext.request.contextPath}/checkImage.action" onclick="change()" title="点击更换验证码">
										</span>
									</td>
								</tr>
							<tr>
